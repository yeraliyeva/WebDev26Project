...
class RedisLeaderboardRepository(AbstractLeaderboardRepository):
    ...
    def get_leaderboard(self, user_id: UUID, top_n: int) -> LeaderboardResult:
        ...
        pipe.zscore(_DAILY_KEY, member)
        raw_top, raw_rank, raw_score = pipe.execute()
        ...
        user_score = int(raw_score) if raw_score is not None else 0
        return LeaderboardResult(top=top, user_place=user_place, user_score=user_score)
